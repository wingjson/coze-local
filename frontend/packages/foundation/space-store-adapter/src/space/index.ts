...
interface SpaceStoreAction {
  ...
  setWorkFlowMode: (workflow_mode: boolean) => void;
  setIsTempl: (workflow_mode: boolean) => void;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        setWorkFlowMode: (mode: boolean) => {
          set({ workflow_mode: mode }, false, 'setWorkFlowMode');
        },
        setIsTempl: (mode: boolean) => {
          set({ isTempl: mode }, false, 'setIsTempl');
        },
        getSpaceId: () => {
          const { id } = get().space;
          if (!id) {
        ...
        fetchSpaces: async (force?: boolean) => {
          ...
          const spaceInfo: SpaceStoreState['spaces'] = {
            ...
            max_team_space_num:
              res?.max_team_space_num ?? DEFAULT_MAXIMUM_SPACE,
          };
